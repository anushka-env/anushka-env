"""
Anushka — neofetch-style info card SVG
"""
import html, os
...
ROWS = [
    ("host",),
    ("kv", "Role",    "Aspiring AI Engineer"),
    ("kv", "Focus",   "Data Science, GenAI, Agentic AI"),
    ("kv", "Edu",     "B.Tech CSE, C.V. Raman Global University"),
    ("kv", "Year",    "2nd Year (3rd Semester) · CGPA: 9.48"),
    ("kv", "Base",    "Bhubaneswar, Odisha, India"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Lang",    "Python, Java, C, SQL"),
    ("kv", "AI/ML",   "Machine Learning, GenAI, Agentic AI (learning)"),
    ("kv", "Tools",   "Git, GitHub, VS Code, Oracle SQL"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "5+ applied Python/Java projects"),
    ("bul", "Strong academic record — CGPA 9.48"),
    ("bul", "Building toward AI Engineering & Data Science"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">anushka@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">anushka</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{KEY_X+124}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
